...
if ads_file and biz_file:
    ...
    st.subheader(f"Next Month Targets (+{roas_uplift:.0%} ROAS | +{organic_lift:.0%} Organic Lift)")
    st.markdown("#### 🌎 Combined Portfolio Overview")
    st.table(format_df(pd.DataFrame([global_total])))
    
    st.divider()
    st.markdown("#### 🏢 Brand-Wise Performance Summary")
    st.table(format_df(proj_df))

    # --- WEEKLY BREAKDOWN (30/20/20/20/10) ---
    st.divider()
    selected_brand = st.selectbox("Select Brand for Weekly Segregation:", options=proj_df['Brand'].unique())
    ...
    st.write(f"### {selected_brand} - Weekly Segregation Table")
    ...
    st.download_button("📥 Download Projection Report", data=output.getvalue(), file_name="Amazon_Growth_Projections.xlsx", use_container_width=True)

else:
    st.info("Upload reports to generate the dashboard. Adjust the uplift filters in the sidebar to update the targets.")
